...
# Get latest SL-Micro AMI (SUSE Linux Micro, BYOS)
# Verified against real AWS account 013907871322 (2026-08-04): naming pattern is
# suse-sle-micro-<major>-<minor>-byos-v<date>-hvm-ssd-x86_64
...
locals {
  # Build FQDN: hostname.subdomain.root_domain (e.g., rancher.rgs-demo-aws.kubernerdes.com)
...
resource "aws_security_group" "rancher" {
  ...
  # RKE2 supervisor / cluster registration for downstream nodes
  ingress {
...
resource "aws_instance" "rancher" {
  ...
  user_data = templatefile("${path.module}/user-data.sh", {
    ...
    rgs_carbide_rancher_chart = var.rgs_carbide_rancher_chart
    rgs_carbide_rancher_image = var.rgs_carbide_rancher_image
