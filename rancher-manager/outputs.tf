...
output "iam_role_arn" {
  description = "ARN of the Rancher EC2 IAM role (useful when wiring up downstream node registration)"
  value       = aws_iam_role.rancher.arn
}

output "ssh_command" {
  description = "SSH command to connect to instance"
  value       = var.ssh_public_key != "" ? "ssh -i ~/.ssh/rgs-demo-aws.pem ec2-user@${var.create_eip ? aws_eip.rancher[0].public_ip : aws_instance.rancher.public_ip}" : "Use AWS Systems Manager Session Manager to connect"
}

output "rancher_url" {
  description = "URL to access Rancher (may take up to 10 minutes to finish bootstrapping)"
